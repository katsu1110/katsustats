...
def _markdown_report(payload: dict[str, object]) -> str:
    ...
    if mc is not None:
        ...
        mc_rows = [
            ...
            ["Sharpe (5th pct)", f"{sh['percentile_5']:.2f}"],
            ["Sharpe (Median)", f"{sh['median']:.2f}"],
            ["Sharpe (95th pct)", f"{sh['percentile_95']:.2f}"],
            ["CAGR (5th pct)", f"{ca['percentile_5']:.2%}"],
            ["CAGR (Median)", f"{ca['median']:.2%}"],
            ["CAGR (95th pct)", f"{ca['percentile_95']:.2%}"],
        ]
        if mc.get("bust_probability") is not None:
            mc_rows.append(["Bust Probability", f"{mc['bust_probability']:.2%}"])
        if mc.get("goal_probability") is not None:
            mc_rows.append(["Goal Probability", f"{mc['goal_probability']:.2%}"])
        lines.extend(
            [
                _markdown_table(["Metric", "Value"], mc_rows),
                "",
            ]
        )
...
def full(
    ...
    Args:
        ...
        monte_carlo: Whether to run Monte Carlo simulation (default False).
...
def _build_html(
    ...
    *,
    ...
    if monte_carlo:
        ...
        if mc_sum["bust_probability"] is not None:
            mc_rows.append(
                (
                    f"Bust Probability (DD ≤ {mc_bust:.0%})",
                    f"{mc_sum['bust_probability']:.2%}",
                )
            )
        if mc_sum["goal_probability"] is not None:
            mc_rows.append(
                (
                    f"Goal Probability (Return ≥ {mc_goal:.0%})",
                    f"{mc_sum['goal_probability']:.2%}",
                )
            )
        mc_table_rows = "".join(
            f"<tr><td>{label}</td><td>{value}</td></tr>" for label, value in mc_rows
        )
        mc_table = (
            f'<table class="metrics"><tr><th>Metric</th><th>Value</th></tr>'
            f"{mc_table_rows}</table>"
        )
        mc_charts = (
            f'<div class="charts-grid">'
        ...
        monte_carlo_section = (
            f'<div class="section">'
            f"<h2>Monte Carlo Analysis ({mc_sims:,} simulations)</h2>"
            f"{mc_table}"
            f"{mc_charts}"
            f"</div>"
        )
